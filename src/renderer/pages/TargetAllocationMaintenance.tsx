...
window.addEventListener("message", (event) => {
    if (event.data.action === "responseData") {
        ...
        if (data.currency) {
            (document.getElementById("currency") as HTMLInputElement).value = data.currency;
        }
...
function addAsset(
    ...
    const form = document.getElementById("assets-form") as HTMLFormElement;
...
function getAssets() {
    ...
    formElements.forEach((element, index) => {
        ...
        const unitPriceInput = element.querySelector("input[name^='unitPrice']") as HTMLInputElement;
        ...
        if (
            ...
            const assetKey = (nameInput as HTMLInputElement).value; // Using the asset name as the key; ensure it's unique
...
function display(labels, oldQuantities, newQuantities, nbsToBuy, targets) {
    var canvas = document.getElementById("stackedChartID") as HTMLCanvasElement;
                ...
                tooltip: {
                    callbacks: {
                        label: function (context) {
                            ...
                            if (label === "Next Buy") {
                                label += `: ${context.raw} (Quantity to buy: ${nbsToBuy[context.dataIndex]
                                    })`;
                            } else {
                                label += `: ${context.raw}`;
                            }
...
function showUpdateButton() {
    (document.getElementById("update-config-button") as HTMLElement).style.display = "block";
}
...
function hideUpdateButton() {
    (document.getElementById("update-config-button") as HTMLElement).style.display = "none";
}
